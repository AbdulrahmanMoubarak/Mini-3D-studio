...
namespace Graphics
{
    class Camera
    {
        ...
        public Camera()
        {
            Reset(5, 2, 6, 0, 0, 0, 0, 1, 0);
            SetProjectionMatrix(45.0f, 1.2f, 0.1f, 100);
        }
        ...
        {
            mDirection = new vec3((float)(-Math.Cos(mAngleY) * Math.Sin(mAngleX) * 7)
                , (float)(Math.Sin(mAngleY) * 3)
                , (float)(-Math.Cos(mAngleY) * Math.Cos(mAngleX)) * 9);
            mRight = glm.cross(mDirection, new vec3(0, 1, 0));
            mUp = glm.cross(mRight, mDirection);

            vec3 center = new vec3(0, 0, 0);//mPosition + mDirection;
            mPosition = mDirection - center;
